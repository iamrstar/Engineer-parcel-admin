...
import { Building, Plus, X, Users, MapPin, Key } from "lucide-react";
...
const Offices = () => {
  ...
  const { login } = useAuth(); // For switching
  ...
  const handleCreateOffice = async (e) => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ name: "", code: "", address: "", contactNumber: "", username: "", password: "" });
      fetchOffices();
    } catch (error) {
  ...
  const handleSwitchToOffice = async (office) => {
    if (!office.adminUser?.username) return toast.error("No valid admin user for this office");
    
    // We cannot securely know the password, so ideally we need an "impersonate" endpoint.
    // For now, prompt admin:
    toast.error("To switch, please logout and login with the office credentials. Impersonation coming soon.");
  };
            ...
            <div key={office._id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
              <div className="p-6 border-b border-gray-50 flex justify-between items-start">
                <div>
                  ...
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black bg-blue-50 text-blue-600 uppercase mt-2">
                    Code: {office.code}
                  </span>
                </div>
              </div>
              <div className="p-6 space-y-4 bg-gray-50/50">
                ...
                <div className="flex flex-col gap-2 pt-4 border-t border-gray-100">
                  ...
                  <button 
                    ...
                    className="w-full py-2.5 bg-gray-900 hover:bg-black text-white text-xs font-bold rounded-xl transition-colors flex items-center justify-center gap-2"
                  >
