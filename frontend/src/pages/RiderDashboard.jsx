                    ...
                    tasks.map((task) => (
                        <div key={task._id} className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 transform transition-all active:scale-[0.98]">
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg border uppercase ${task.assignedFor === 'pickup' ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-purple-50 text-purple-700 border-purple-100'
                                        ...
                                        {task.serviceType === 'campus-parcel' && task.assignedFor === 'delivery' ? 'Box Delivery' : task.assignedFor}
                                    </span>
                                </div>
                            ...
                            <div className="space-y-4">
                                {task.assignedFor?.toLowerCase() !== 'delivery' && (
                                ...
                                {task.assignedFor?.toLowerCase() !== 'pickup' && (
                                    <div className="flex gap-4">
                                        <div className="flex flex-col items-center">
                                            <div className={`h-8 w-8 rounded-xl flex items-center justify-center ${task.serviceType?.toLowerCase().trim() === 'campus-parcel' ? 'bg-purple-100' : 'bg-blue-100'}`}>
                                                {task.serviceType?.toLowerCase().trim() === 'campus-parcel' ? <Truck className="h-4 w-4 text-purple-600" /> : <Navigation className="h-4 w-4 text-blue-600" />}
                                            </div>
                                        </div>
                                        <div className="flex-1">
                                            <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">
                                                {task.serviceType?.toLowerCase().trim() === 'campus-parcel' ? 'Deliver Boxes To' : 'Deliver To'}
                                            </p>
                                            <p className="text-sm font-bold text-gray-800">
                                                {task.serviceType?.toLowerCase().trim() === 'campus-parcel' ? task.senderDetails?.name : task.receiverDetails?.name}
                                            </p>
                                            <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">
                                                {task.serviceType?.toLowerCase().trim() === 'campus-parcel' ? task.senderDetails?.address : task.receiverDetails?.address}
                                            </p>
                                            <a 
                                                href={`tel:${task.serviceType?.toLowerCase().trim() === 'campus-parcel' ? task.senderDetails?.phone : task.receiverDetails?.phone}`} 
                                                className={`inline-flex items-center gap-2 mt-2 font-bold text-xs px-3 py-1.5 rounded-lg ${task.serviceType?.toLowerCase().trim() === 'campus-parcel' ? 'text-purple-600 bg-purple-50' : 'text-blue-600 bg-blue-50'}`}
                                            >
                                                <Phone className="h-3 w-3" /> Call {task.serviceType?.toLowerCase().trim() === 'campus-parcel' ? 'Sender' : 'Receiver'}
                                            </a>
                                        </div>
                                    </div>
                                )}
